...
- 🖥️ Screen sharing & presentation mode:
  ...
  - 🧊 **Floating Control Dock** (Electron desktop only, window shares) — since click-to-focus backgrounds BeeHive's main window, a separate **always-on-top** window keeps the core meeting controls reachable above whatever app is now in front:
    ...
    - **Actually clickable while a full-screen app has focus — without collapsing the presentation** — three layered fixes for the same family of symptoms. (1) `webPreferences.acceptsFirstMouse: true`: macOS treats a click on an *inactive* window purely as "activate this window" and swallows it, so every dock button looked dead on the first click while Keynote/PowerPoint held focus. (2) `setIgnoreMouseEvents(false)` reasserted on every window (re)creation as a guard against any stuck ignore-state (the window object is reused across show/hide cycles). (3) The load-bearing one: **`type: 'panel'`** — a normal window *activates its owning app* on mouse-down (before and regardless of `acceptsFirstMouse`, which only controls whether that same click also reaches the button), and activating BeeHive while Keynote/PowerPoint is mid-slideshow makes macOS drop the presentation out of full screen — so every dock click, even mute, was collapsing the show back to a window. A non-activating NSPanel receives clicks without ever activating BeeHive — the same mechanism screen-recorder control bars (Loom etc.) use to stay clickable over full-screen apps
