...
- 🖥️ Screen sharing & presentation mode:
  ...
  - 🧊 **Floating Control Dock** (Electron desktop only, window shares) — since click-to-focus backgrounds BeeHive's main window, a separate **always-on-top** window keeps the core meeting controls reachable above whatever app is now in front:
    ...
    - **Actually clickable while a full-screen app has focus** — the dock is shown via `showInactive()` so it never steals focus from the presented app, but by default macOS treats a click on an *inactive* window purely as "activate this window" and swallows it — every dock button looked dead on the first click while Keynote/PowerPoint held focus. Fixed with `webPreferences.acceptsFirstMouse: true`, which delivers that first click straight to the button instead of consuming it on activation. Two more explicit assertions applied for the same "visible but unclickable over another app's true full-screen Space" symptom: `focusable: true` (a non-focusable window can be shown above a full-screen space but macOS won't reliably deliver it real mouse-down events) and `setIgnoreMouseEvents(false)` reasserted every time the window is (re)created (the window object is reused across show/hide cycles, so this guards against any stuck ignore-state rather than assuming the default holds). Note: true OS-level full-screen Spaces are an exclusive input context macOS manages per-app — these are every configuration flag Electron/macOS expose for an auxiliary overlay to participate in another app's full-screen space, but final behavior can still depend on how the presenting app itself manages its full-screen window
