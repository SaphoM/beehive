...
- 🖥️ Screen sharing & presentation mode:
  ...
  - 🧊 **Floating Control Dock** (Electron desktop only, window shares) — since click-to-focus backgrounds BeeHive's main window, a separate **always-on-top** window keeps the core meeting controls reachable above whatever app is now in front:
    ...
    - **Actually clickable while a full-screen app has focus — without collapsing the presentation** — three layered fixes for the same family of symptoms. (1) `webPreferences.acceptsFirstMouse: true`: macOS treats a click on an *inactive* window purely as "activate this window" and swallows it, so every dock button looked dead on the first click while Keynote/PowerPoint held focus. (2) `setIgnoreMouseEvents(false)` reasserted on every window (re)creation as a guard against any stuck ignore-state (the window object is reused across show/hide cycles). (3) **`type: 'panel'` + `focusable: false` together** — a normal window *activates its owning app* on mouse-down (before and regardless of `acceptsFirstMouse`, which only controls whether that same click also reaches the button), and activating BeeHive while Keynote/PowerPoint is mid-slideshow makes macOS drop the presentation out of full screen. The non-activating NSPanel fixes that half — but wasn't sufficient alone: a non-activating panel still becomes the **key window** system-wide when clicked, taking key status away from Keynote's playback window *without* activating BeeHive, and Keynote treats its playback window resigning key as "end the show" — so clicks still collapsed the presentation. `focusable: false` means the dock can never become key at all: clicks are delivered purely as mouse events (all any dock button needs — nothing here takes keyboard input) and the presenting app keeps key status through every interaction. This panel + non-focusable combination is the actual recipe screen-recorder control bars (Loom etc.) use to stay clickable over full-screen apps. Two further Space-system exemptions complete it: `setVisibleOnAllWorkspaces(..., { skipTransformProcessType: true })` — without the skip, joining all workspaces works by *transforming the app's process type* (regular ⇄ UIElement) under the hood, a transform documented to cause app-activation/Space side effects, i.e. interacting with the dock could yank macOS away from the presentation's full-screen Space, ending the slideshow — and `hiddenInMissionControl: true`, keeping the dock out of Mission Control/the Spaces system entirely
...
### Attendees Strip
- Click the participant count pill in the header to open — works in **every view**: standard desktop, mobile (previously disabled there entirely — the header pill's click handler explicitly no-op'd on mobile, so there was no way to see attendees on a phone at all), and full-screen (see `FullscreenHud` above, which reuses this same list)
- Always a single, fixed layout: a **horizontal, always-scrollable strip docked directly under the header**, full width — never a draggable floating window. This used to be two components (`ParticipantsWindow`, a draggable grid window you had to drag onto the header to get this layout, plus `DockedParticipantsStrip` for after that drag) — collapsed into one, since the strip is now the only attendees view there is; no drag gesture required to reach it
...
- **✕** closes it
- **Opens only on an explicit click, never on its own** — a previous version force-opened this the instant any remote screen share started, keyed on `hasRemoteScreenShare`; that value flips false→true→false→true across ordinary presenter actions (switching the shared window/source, a brief resubscribe hiccup), and every false→true edge re-triggered the open — even right after a viewer had explicitly closed it, which read as "won't stay closed." Removed entirely
- **Closes itself once sharing actually ends** (both a local share stopping and a remote share ending) — the reverse direction is safe in a way forcing it open wasn't: this can only ever turn the strip off, never force it back on, so it can't reintroduce the bug above. If you had it open on purpose before the share ended, closing it there is just tidying up after the share it was likely opened to check in on
...
│   ├── ParticipantsWindow.tsx          #   attendees strip (horizontal, always-scrollable, under header); host-only co-host toggle
