...
- **Both platforms ship their native installer directly** — each link points at a **direct-download asset URL on a GitHub Release** (`NATIVE_MAC_DMG_URL` / `NATIVE_WIN_EXE_URL` in `DesktopHandoff.tsx`), not the Releases *page*, so clicking it starts the download immediately with no click-through:
  ...
  - **Windows** → `BeeHive-Setup-x64.exe` (x64 — the standard architecture for the vast majority of Windows PCs; an arm64 build is also published for Windows-on-ARM devices but isn't linked from the UI, matching `detectDesktopOS()`'s OS-level, not architecture-level, detection)
  ...
  - **Stable, version-less filenames on GitHub's `latest` release alias** — both URLs above resolve via `releases/latest/download/<name>`, which always serves whatever asset with that exact name is attached to the most recently published release. `package.json`'s `build.mac.artifactName` / `build.win.artifactName` emit exactly these fixed names (no version baked into the filename), which is what makes this possible: the old scheme (`BeeHive-1.0.0-arm64.dmg`, tied to a specific version tag) went stale on every version bump and needed the download URL hand-edited in `DesktopHandoff.tsx` each time. This URL never needs to change again.
  - To publish a new build: `npm run electron:build:mac` and `npm run electron:build:win` (the latter needs an explicit `--x64` or `--arm64` flag — see `package.json`'s `build.win.artifactName`), then `gh release upload latest release/<dmg-or-exe-path> --clobber` to replace the asset on the `latest`-aliased release in place — the download URL never needs touching.
...
npm run electron:build:win   # → release/BeeHive-Setup-x64.exe (NSIS installer)
```
...
- **A packaging misconfiguration blocked the universal merge outright**: the first universal build attempt failed with `@electron/universal` refusing to merge, because `node_modules/@rolldown/binding-darwin-arm64/*.node` (Vite 8's native Rust bundler binary) was present in the arm64 build's `app.asar.unpacked` but not the x64 build's — an arch-specific native binary that's identical-looking-but-arch-mismatched trips `@electron/universal`'s safety check by design. Root cause: `vite` and `@vitejs/plugin-react` were listed under `package.json`'s `dependencies` instead of `devDependencies` — pure build-time tooling that should never have been included in the packaged app at all (the shipped app only ever runs `dist/**` static output + the bundled backend; it never imports Vite at runtime). Moved both to `devDependencies`, where electron-builder's dependency-tree walk correctly excludes them from the packaged `node_modules`. This also trims unnecessary size from every build, independent of the universal-merge fix.
...
**App versioning — derived from git, not hand-edited:** the version shown in the app (bottom-right corner, next to "Built by X Spark" — see `BuiltByFooter.tsx`) and baked into every packaged build is `1.0.<total commit count>`, e.g. `1.0.193` for a repo with 193 total commits.
- **Computed by `scripts/appVersion.mjs`**: `getAppVersion()` runs `git rev-list --count --all` and returns `1.0.${count}` — monotonic, automatic, and ties every build to an exact point in history without anyone needing to remember to bump a number. `getAppSha()` similarly exposes the short commit hash (`git rev-parse --short HEAD`), for cases where the exact build matters more than the count.
- **For the web app**: `vite.config.ts` imports both and injects them via `define: { 'import.meta.env.VITE_APP_VERSION': ..., 'import.meta.env.VITE_APP_SHA': ... }` — a real build-time constant, not a runtime lookup, so it works identically in dev and in the deployed production bundle. `BuiltByFooter.tsx` renders it in dim grey (`v1.0.193`) beside the brand credit.
...
- This has no relationship to the desktop installers' *filenames* (`BeeHive-universal.dmg` / `BeeHive-Setup-x64.exe`), which are deliberately version-less/stable so the GitHub `releases/latest/download/...` links never go stale — see the Download section above.
- **Shallow-clone bug, fixed**: the deployed web app briefly showed `v1.0.1` instead of the real count — Render's build clones with `git clone --depth 1`, so `git rev-list --count` doesn't error, it just silently walks the one commit that's actually present. `getAppVersion()` checks `git rev-parse --is-shallow-repository` first and runs `git fetch --unshallow` before counting if so (memoized, so it only runs once per build even though `vite.config.ts` calls both `getAppVersion()`/`getAppSha()`). Verified directly: forced a real shallow clone of this repo via `git clone --depth 1 file://...`, confirmed it reproduced the exact `1.0.1` bug beforehand, then confirmed the fixed script unshallows it and returns the correct full count.
- **Branch-relative undercount, fixed**: the count originally came from `git rev-list --count HEAD`, which only counts commits reachable from *whichever branch is checked out* — confirmed this made the number wildly inconsistent across this repo's own branches at the same point in time (`main` → 1, `develop` → 20, a feature branch → 18/21, `staging` → 193), so a build made from `main` would have shown `v1.0.1`, an apparent (and completely false) massive downgrade from staging's `v1.0.193`. Switched to `git rev-list --count --all`, which counts unique commits across every ref in the repository (every local/remote branch, every tag) rather than just the checked-out branch's ancestry — verified directly: checked out both `main` and `staging` and confirmed `--all` reports the identical `193` on both, whereas the old `HEAD`-based count did not.
