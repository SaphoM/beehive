...
- **Participant panel row height**: `components/roomStyles.ts`'s `dockedTile` (the compact strip both the floating `ParticipantsWindow` and `DockedParticipantsStrip` share) went from `70px` to `105px` tall (+50%, width untouched — only height was asked for), its avatar from `34px` to `52px` so it reads as the row's focal point, and `pwWindow` got a `minHeight: 220` so the floating panel itself is visibly more spacious even with few participants, matching the Teams/Meet reference look.
  - **Tiles now wrap to actually use that height**: the taller panel left a single row of small tiles floating in a lot of empty space below them once there were only one or two attendees. New `dockedInnerWrap` style (vs. the original `dockedInner`) lets tiles reflow onto additional rows as more participants join, turning the floating panel into a proper grid instead of a single strip with dead space beneath it — gated by a new `wrap` prop on the shared `AttendeeTiles`, deliberately **not** applied to `DockedParticipantsStrip` (the header-docked variant), which stays exactly the slim single-row strip it always was — there's no spare vertical room there to wrap into, and wrapping it would have grown the header area unexpectedly.
