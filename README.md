...
**App versioning — derived from git, not hand-edited:** the version shown in the app (bottom-right corner, next to "Built by X Spark" — see `BuiltByFooter.tsx`) and baked into every packaged build is `1.0.<commit count>`, e.g. `1.0.189` for a checkout with 189 commits on it.
- **Computed by `scripts/appVersion.mjs`**: `getAppVersion()` runs `git rev-list --count HEAD` and returns `1.0.${count}` — monotonic, automatic, and ties every build to an exact point in history without anyone needing to remember to bump a number. `getAppSha()` similarly exposes the short commit hash (`git rev-parse --short HEAD`), for cases where the exact build matters more than the count.
- **For the web app**: `vite.config.ts` imports both and injects them via `define: { 'import.meta.env.VITE_APP_VERSION': ..., 'import.meta.env.VITE_APP_SHA': ... }` — a real build-time constant, not a runtime lookup, so it works identically in dev and in the deployed production bundle. `BuiltByFooter.tsx` renders it in dim grey (`v1.0.189`) beside the brand credit.
