...
- **macOS ships the native installer directly:** the link points at the `.dmg`'s direct-download URL on a **GitHub Release** (`https://github.com/SaphoM/beehive/releases/download/v1.0.0/BeeHive-arm64.dmg`) — a direct asset link, not the Releases page, so clicking it starts the download immediately with no click-through. The `.dmg` is 500MB+ (over GitHub's 100MB per-file git limit), which is why it's published as a release asset rather than committed to the repo. To publish a new build: `npm run electron:build:mac`, then `gh release create v<version> release/BeeHive-<version>-arm64.dmg#BeeHive-arm64.dmg --title "BeeHive v<version> (macOS)"` (or `gh release upload` onto an existing tag), and bump `NATIVE_MAC_DMG_URL` in `DesktopHandoff.tsx` if the tag changed. Override for a differently-hosted binary with `VITE_DESKTOP_DOWNLOAD_MAC_URL`
