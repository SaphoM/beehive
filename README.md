...
- **Both platforms ship their native installer directly** — each link points at a **direct-download asset URL on a GitHub Release** (`NATIVE_MAC_DMG_URL` / `NATIVE_WIN_EXE_URL` in `DesktopHandoff.tsx`), not the Releases *page*, so clicking it starts the download immediately with no click-through:
  - **macOS** → `BeeHive-arm64.dmg` (Apple Silicon)
...
```bash
npm run electron:build:mac   # → release/BeeHive-arm64.dmg  (Apple Silicon)
...
- This has no relationship to the desktop installers' *filenames* (`BeeHive-arm64.dmg` / `BeeHive-Setup-x64.exe`), which are deliberately version-less/stable so the GitHub `releases/latest/download/...` links never go stale — see the Download section above.
...
1. Open `release/BeeHive-arm64.dmg`
