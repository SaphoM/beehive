...
- **Meeting-type cards** — a responsive, scrollable grid of ~23 templates (Board, Client, Follow-up, Brainstorm, Project Kickoff, Team Stand-up, Sales, Training, Interview, Executive, Workshop, One-on-One, Performance Review, Sprint Planning / Review / Retro, Investor, Product Demo, Discovery Call, Quarterly Review, Annual Planning, Technical Meeting, Support Call). Each card is a simple monochrome `lucide-react` icon + title + description — consistent with the rest of the app's icon language rather than emoji — keyboard-focusable (`aria-pressed`, focus ring), touch-friendly, and lifts on hover
...
#### Avatars — initials fallback + profile picture upload

Every user gets a professional circular avatar everywhere a name is shown (chat, waiting room, the participants strip, the fullscreen HUD's attendee list, the lobby's own self-display) — a real photo if they've uploaded one, otherwise deterministic initials on a deterministic colour. No broken-image icons, no blank circles, ever.

- **`components/Avatar.tsx`** is the single shared implementation — every call site renders the exact same `<Avatar name avatarUrl size/>`, so there's no risk of one spot showing stale initials while another shows the real photo.
  - `getInitials(name)` — first + last word's first letter (`"Sapho Maqhwazima"` → `"SM"`, `"John Smith"` → `"JS"`, `"Mary Ann Brown"` → `"MB"`, matching every reference app's convention of ignoring middle words); a single-word name gets just its first letter (`"Cher"` → `"C"`); punctuation and repeated whitespace are stripped with a Unicode-aware regex (`\p{L}`/`\p{N}` with the `u` flag), and the string is iterated by codepoint (`[...str]`) rather than UTF-16 code unit, so composed international characters survive intact (`"José García"` → `"JG"`). Verified directly against all four of the reference app's example names plus the edge cases (empty, `null`, punctuation-heavy, international) in a live browser — every one matched.
  - `getAvatarColor(name)` — a simple string hash picks one of eight curated, soft, high-contrast-with-white colours (blue/purple/green/orange/teal/rose/indigo/sea-green). A curated palette, not an arbitrary `hsl(hash % 360, ...)` generator, avoids the muddy/neon results plenty of hash values produce with an unconstrained hue — and the hash makes the same name always land on the same colour, verified directly (`getAvatarColor(name) === getAvatarColor(name)`).
  - `<Avatar/>` renders the real image when `avatarUrl` is set and loads successfully; falls back to the initials circle immediately — including on an `<img onError>` (a broken/expired URL never shows a broken-image icon, it just quietly becomes the initials version instead).
- **Upload** (`uploadAvatar()` in the same file): validates MIME type (JPG/JPEG/PNG/WEBP only, clean rejection message otherwise — verified directly, a `.txt` file is rejected before any network call happens), then processes entirely via Canvas 2D (no new image-processing dependency — this app already relies on Canvas for the virtual-background pipeline): center-crops to square, resizes to 320px, and re-encodes as JPEG at 0.85 quality. A transparent PNG (an explicit test case) gets a white backdrop filled in before the crop, so it doesn't flatten to a black square when re-encoded to a format with no alpha channel — verified directly with a synthetic transparent, non-square PNG. "Center the face where practical" is implemented as a plain center crop; true face-detection cropping would be a meaningfully larger feature and is out of scope here.
  - Uploads to a **new** `avatars` Supabase Storage bucket (`supabase/migrations/005_avatars_bucket.sql`), not the existing `shared-files` bucket — that bucket's upload policy has no auth check at all (`with_check: bucket_id = 'shared-files'`, fine for ephemeral in-meeting file shares, wrong for profile pictures, which need to be restricted to their owner). `avatars` is public-read (so `profiles.avatar_url` renders as a plain `<img src>` everywhere with no auth round-trip) but insert/update/delete are restricted, via `storage.foldername(name)[1] = auth.uid()`, to a user's own `{userId}/...` path — verified directly: an upload attempt using a fake/mismatched user id was rejected with "new row violates row-level security policy" at the storage layer, not merely gated by client-side UI.
  - `EditableAvatar` wraps `<Avatar/>` with both requested entry points: clicking the avatar opens a small menu ("Upload profile picture", plus "Remove profile picture" once one exists) — that's Method 1. `showChangeLink` renders a plain secondary "Change profile picture" text link beside it, wired to the same upload path — intended as Method 2, adapted from the mission's "within the user profile/settings page" phrasing since this app has no dedicated settings page to host a button on (adding one would be new navigation/layout, out of scope here); it's placed directly beside the avatar it controls instead. Removing a picture just clears `profiles.avatar_url` back to `null` — every `<Avatar/>` call site already treats that identically to "never uploaded one," so the initials avatar reappears everywhere with no special-case revert logic needed.
- **Showing other participants' real photos, not just their own**: `room_participants.auth_user_id` has no foreign-key constraint to `profiles` (only the legacy, unused `user_id` → the old `users` table does), so PostgREST can't embed `profiles(avatar_url)` in one query the normal way without adding a new constraint. `useParticipants` (`livekit_react_hooks.tsx`) instead does a small second lookup — the distinct `auth_user_id`s actually present in the room's participant list, queried against `profiles` and merged in client-side — rather than adding a DB constraint just for this. Guests (`auth_user_id` null) simply never get one, which `<Avatar/>` already treats the same as "no picture yet." Known limitation: this doesn't live-update if someone changes their photo *mid-meeting* (no subscription to `profiles`, only to `room_participants`) — acceptable given how rarely that would happen mid-call, and any other `room_participants` change (role, active state) already re-triggers the merge and would pick up a stale avatar's replacement anyway.
- **Main meeting grid, camera off** (`components/CameraOffAvatarOverlay.tsx`): the one real remaining gap when avatars first shipped — `RoomPage.tsx`'s `<GridLayout tracks={cameraTracks}><ParticipantTile /></GridLayout>` uses LiveKit's own default tile rendering, which draws its own built-in grey person-silhouette placeholder (`ParticipantPlaceholder`) whenever a camera is off. `ParticipantTile`'s `children` prop *replaces* its entire default body (video + placeholder + name badge + mic icon + connection quality all at once — confirmed by reading the library's own source, `children ?? <defaultBody/>`), so rebuilding all of that by hand to swap in just the placeholder would risk regressing the tile's existing look. Instead: `GridLayout` clones whatever single child it's given once per track and wraps each clone in a per-tile track-reference context (the same mechanism its own docs describe for using `ParticipantTile` as a child of `TrackLoop`) — so wrapping `<ParticipantTile />` in a plain host `<div>` alongside `<CameraOffAvatarOverlay/>` means both receive that same context, and the overlay can read `isTrackReferencePlaceholder()` (`@livekit/components-core`) to detect "no real track" and render `<Avatar/>` on top, purely additively, with zero changes to `ParticipantTile`, `GridLayout`, or any track/WebRTC logic.
- **Participant panel row height**: `components/roomStyles.ts`'s `dockedTile` (the compact strip both the floating `ParticipantsWindow` and `DockedParticipantsStrip` share) went from `70px` to `105px` tall (+50%, width untouched — only height was asked for), its avatar from `34px` to `52px` so it reads as the row's focal point, and `pwWindow` got a `minHeight: 220` so the floating panel itself is visibly more spacious even with few participants, matching the Teams/Meet reference look.
  - **Tiles now stretch to actually fill that height**: a flex-wrap version of the tile row still left a lot of empty space with only one or two attendees — wrapping alone doesn't make existing rows any taller. `dockedInnerWrap` is a real CSS Grid instead (`grid-template-columns: repeat(auto-fill, minmax(140px, 1fr))`, `grid-auto-rows: minmax(105px, 1fr)`): with `1fr` row tracks, grid's default `align-content: stretch` distributes all of the panel's free vertical space into however many rows are actually needed, so one participant's tile fills the whole panel, and each tile itself switches to `width/height: 100%` (from its old fixed `110×105`) to fill whatever cell size the grid gives it — verified visually at both 1 and 6 participants. Gated by a new `wrap` prop on the shared `AttendeeTiles`, deliberately **not** applied to `DockedParticipantsStrip` (the header-docked variant), which stays exactly the slim single-row strip it always was — there's no spare vertical room there to fill, and doing this there would have grown the header area unexpectedly.
  - **Padding, so tiles don't touch the panel's border**: `dockedInnerWrap` initially had no `padding` of its own — with the grid filling 100% of the panel's available space, a tile's own border could sit flush against `pwWindow`'s border with nothing between them. Added `padding: 12` directly on the grid container, verified visually (highlighted the panel's border to confirm a clear gap on every side, not just eyeballed).
- **Speaking-indicator floating window** (`components/SpeakingIndicator.tsx`): this is the other floating self/speaker-preview box that shows during a call (title = speaker's name, minimise/close, a video-or-placeholder area) — it had the exact same gap as the main grid, showing a plain `VideoOff` icon instead of an avatar whenever the active speaker's camera was off. Now renders `<Avatar/>` there instead, matching every other camera-off spot in the app. Needed a `fullName` field added alongside the existing (intentionally first-word-only) `name` used for the compact chip, since avatar lookup needs the real `display_name`, not `"Sapho"` out of `"Sapho Maqhwazima"`.

#### BeeHive Bot Assistant — live, shared meeting agenda

A floating assistant button (bottom-right, clear of the centred control bar) that expands into a panel for managing the meeting's agenda in real time — every participant sees the same, live state, not a per-device snapshot.

- **Deliberately separate from the existing "meeting prep" feature** (`SchedulePanel` → `MeetingPrep` → `MeetingPrepWindow`, persisted in `localStorage` under `beehive:prep:<roomId>` — see `roomUtils.ts`'s `saveMeetingPrep`/`loadMeetingPrep`). That feature is per-device and read-only once in the room — fine for its original purpose (a checklist set once at scheduling time), wrong for something every participant needs to see update live, including Start Now meetings, which never go through scheduling-time prep at all. `components/BotAssistant.tsx` is new, additive code with its own new table — the existing prep feature, its component, and its storage are untouched.
- **New `meeting_agendas` table** (`supabase/migrations/006_meeting_agenda.sql`) — one row per room: `title`, `organizer_name`, `objectives text[]`, `items jsonb` (`{ id, text, completed, notes, order }[]`). Permissive `SELECT` (anyone in the room reads the live agenda, matching every other room-scoped realtime table here), **no** `INSERT`/`UPDATE`/`DELETE` policy for anon/authenticated at all — mirrors `admission_requests`/`room_hosts`. Verified directly, not assumed: a raw `PATCH` via the anon key against a real row returned `204` (PostgREST's default response even when RLS filters the update to zero matched rows) but the row's `title` was confirmed unchanged afterward — the write was genuinely rejected, the HTTP status alone would have been misleading.
- **`POST /api/rooms/:roomId/agenda`** (`livekit_node_backend.js`) is the only way this table is ever written. Authorized via `isRoomModerator()` — the exact same helper `/admit` and `/mute` already use, not a new permission system. Same scope as every other moderation feature this session: only meaningful in a waiting-room-gated (scheduled) meeting where a host/co-host actually exists; an ungated Start Now meeting has no host concept at all, so nobody currently holds edit rights there either — a pre-existing, consistent limitation, not something new introduced here. Verified end-to-end: scheduled a real room, wrote an agenda with the returned `hostSecret`, confirmed the row landed in the database with the exact content sent; separately confirmed a request with no `hostSecret`/moderator role gets `403`, and a malformed body gets `400`.
- **`useMeetingAgenda(roomId)`** (`livekit_react_hooks.tsx`) is the single realtime subscription — one source of truth per room. The panel keeps a local `draft` for responsive typing, debounced (600ms) into the `POST` above, and only re-syncs `draft` from the live subscription when there's no save in flight, so an incoming realtime update can't clobber an unsent keystroke.
- **RBAC**: edit controls (title/objectives/item text/notes/add/delete/reorder) only render for `canAdmit` (host/co-host) — participants without that role get the exact same panel with read-only text instead of inputs, never a different/degraded view.
- **AI-assist is local heuristics, not a real model call** — "Suggest structure" (a canned template, only fills if the agenda is empty), "Generate objectives" (derives phrases from existing item text), "Estimate time" (evenly divides a prompted total across items). No LLM integration exists anywhere in this codebase yet; these are isolated in one clearly-labelled place in `BotAssistant.tsx` (`AI-ASSIST HEURISTICS`) specifically so swapping any one for a real model call later is a localized change.
- **Bug, found immediately after shipping: the scheduling-time agenda never actually reached the assistant.** `SchedulePanel.tsx`'s prep template/agenda only ever went into `saveMeetingPrep()` — per-device `localStorage`, read back by the *existing*, separate `MeetingPrepWindow`. The new shared `meeting_agendas` table started empty for every scheduled meeting, so the assistant always showed "No agenda has been set," even for a meeting scheduled with a full 8-item template — confirmed directly against a real screenshot of the app. Worse than just "not yet synced": Electron's `file://` origin can never see a web tab's `localStorage` at all, so even fixing this by reading local storage from `BotAssistant` itself would have left the desktop app with nothing regardless of device. Fixed at the source instead — `SchedulePanel.tsx`'s `handleCreate`/`sendEmails` now also call a new `seedSharedAgenda()`, which `POST`s the picked template's title/agenda straight into `meeting_agendas` via the same `/api/rooms/:roomId/agenda` endpoint, using the `hostSecret` `useScheduleRoom` already mints (now returned directly from `scheduleRoom()`, not just stashed in `localStorage`, so it's usable in the same tick without a redundant round-trip). The template's `goals` (labelled "Objectives" in `MeetingPrep.tsx`'s own UI) aren't part of the existing `MeetingPrepSummary` type at all — looked up fresh via the already-exported `getTemplate(prep.templateId)` rather than duplicating that field into a working, unrelated type. Verified end-to-end for real: scheduled an actual room, seeded it exactly as `SchedulePanel` would, and confirmed the database row matches the sent content exactly, not just a `200`.
- **Second bug behind the same symptom — the host was landing in the wrong room entirely.** After the seed fix above, scheduled rooms genuinely had agendas (verified in the DB), yet the assistant still showed "No agenda has been set" in the live meeting. Root cause was the post-schedule *user flow*, not the data: `SchedulePanel`'s "Create Meeting & Get Link" only ever gave the host a copy-link / send-invite path — **there was no in-app button to actually start the room they'd just scheduled**. So the host fell back to the obvious "Start [meeting]" button, which is `onCreateRoom` → a fresh **Start Now** room (`requires_admission: false`, auto-named "Meeting H:MM:SS"), with no agenda. The database proved it exactly: the scheduled rooms (`requires_admission: true`) had the seeded agendas, while the auto-named Start Now rooms the host was actually joining had none. Fixed by adding a **"Start meeting now"** button to `SchedulePanel`'s post-create block, wired through `Lobby` (`onStartScheduled`) to a new `handleStartScheduled` in `RoomPage` that joins the *scheduled* room by id — the host holds that room's `hostSecret` (saved to `localStorage` by `useScheduleRoom` at creation), so `joinRoom` admits them immediately rather than into the waiting room, and they land in the room that already holds the seeded agenda. Refactored the existing `handleJoin` and this new path onto one shared `enterRoom(targetRoomId)` core rather than duplicating the join/pending/ended/error handling. Verified the read side too, not just the write: ran the exact query `useMeetingAgenda` issues (`select title,organizer_name,objectives,items,updated_at … eq room_id`) against a real seeded room *through the anon key* (what the browser actually uses) and confirmed it returns the full agenda (`200`, 6 items, 4 objectives) — so with the host now routed into that room, the assistant has real data to render.
...
- 🎙️ **Background-noise suppression** — the mic is automatically run through LiveKit's Krisp noise-filter processor, which filters ambient/background noise and isolates the speaker's voice; applied to every mic track this participant publishes (including after toggling the mic off and back on, which creates a fresh track); the ~5–6 MB Krisp WASM/ML payload is **lazy-loaded** on first mic publish via dynamic `import()` — it never bloats the initial page load; silently skipped on unsupported browsers/platforms, so it never blocks the mic
  - **Mic capture tuning**: `LiveKitRoom`'s `audioCaptureDefaults` explicitly sets `channelCount: 1` — LiveKit's own baseline already turns on `autoGainControl`/`echoCancellation`/`noiseSuppression`/`voiceIsolation` (confirmed by reading `livekit-client`'s source directly; this app was already close to what browsers expose before touching anything here), so mono capture is the one genuinely additive, low-risk constraint layered on top — avoids stereo-channel comb-filtering/phase artifacts some hardware introduces on a "stereo" input that's really a single voice source. Applies to every future mic enable, not just the initial join (`audioCaptureDefaults` is merged into every `setMicrophoneEnabled()` call by `livekit-client`, not just the first one).
  - **Same-Room Audio Coordination** (opt-in, per-meeting) — `components/RoomAudioCoordination.tsx`. When BeeHive suspects two or more attendees are physically in the same room, a small dismissible prompt offers to coordinate their mics: the current speaker (among those who opt in) stays fully open, everyone else opted-in is gently *ducked* (−18dB, not muted) while quiet, restoring within an 80ms ramp the instant they start speaking. Never a large dialog, never persistent, never applied without explicit per-meeting consent.
    - **Detection is a real heuristic, not a stub, but its real-world accuracy is unverified here**: `components/useSameRoomDetection.ts` computes a rolling Pearson correlation between the local mic's loudness envelope and each remote participant's — the same physical fact that causes acoustic feedback in the first place (two mics in the same room pick up the same voice's loudness contour at the same instants; two mics in different rooms don't). Verified directly that the correlation math itself is sound (identical envelopes → 1.0, envelope + independent noise → ~0.999, two unrelated random series → ~−0.16, well separated from the 0.55 threshold) — what's **not** verified is whether real acoustic coupling between two physical microphones actually produces correlations above that threshold in practice; there's no real audio hardware in this environment to test that empirically. It only ever produces a dismissible suggestion — enabling coordination is always an explicit user action, and a sustained (2s+) correlation is required before suggesting anything at all, not a single lucky reading.
    - **Ducking, not replacing, Krisp**: `components/RoomCoordinationProcessor.ts` is a composite `TrackProcessor` that runs the *exact same* Krisp filter internally, then adds a `GainNode` after it — raw mic → Krisp (byte-for-byte unchanged) → gain (this feature's only addition) → published track. This is deliberate: LiveKit's `LocalTrack.setProcessor()` holds exactly one processor slot, so calling it a second time to add ducking would have silently *replaced* Krisp instead of extending it. The composite processor is only ever installed while a participant has coordination enabled; everyone else keeps the plain `KrispNoiseFilter()` this app already shipped, completely untouched. Krisp's package is imported dynamically inside this file (not statically) for the same reason RoomPage.tsx's existing Krisp effect already lazy-loads it — a static import here was caught during verification inflating the built bundle from ~1.3MB to ~7MB by pulling Krisp's WASM payload into the main chunk eagerly; fixed before shipping.
    - **Every mic stays under its own owner's control** — a participant's client only ever adjusts *its own* gain node; nothing here gives any client the ability to touch another participant's track. Manual mute is a natural no-op (a muted mic has no track to duck). Host/co-host force-mute (the moderation feature's `/mute` endpoint) happens at the LiveKit SFU level, entirely independent of this local gain graph, and wins regardless.
    - Who has coordination enabled this meeting is tracked via a plain Supabase Realtime broadcast (`room-coordination:{roomId}`) — the same broadcast pattern already used for raised hands and moderation elsewhere in this file, not a new mechanism.
    - **Verified in this environment**: the gain-ramp mechanics (source → gain → destination graph, ~80ms ramp down to the ducked level and back) work exactly as designed, timed directly in a real browser `AudioContext`. **Not verified here** (no real microphone in this environment): Krisp's own `init()` against a real `getUserMedia` capture (it internally calls `track.applyConstraints()` on the input, which a synthetic test track doesn't support the same way a real mic track does), the real-world accuracy of the same-room correlation threshold, and perceived audio quality generally — all four require a human with live audio hardware to confirm before treating this as fully proven, and that's stated plainly rather than claimed as done.
